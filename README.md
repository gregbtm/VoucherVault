...
- 🤖 **AI-assisted "Scan with AI"** — snap a photo of a physical voucher, coupon, or gift card, *or* upload a screenshot (an emailed gift card, a retailer app screen, a digital wallet pass) and let an OCR backend pre-fill the redeem code, barcode type, merchant, and expiry date for you. Three interchangeable backends: Claude's vision model, OpenAI's vision model, or a fully local, free Tesseract backend with no API key at all.
