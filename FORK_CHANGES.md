...
sixteen rounds of additive features, from Phase 1 (a REST API) through
...
illustration pass over everything that came before it), and Phase 16 (a
one-click Portainer redeploy webhook), plus several standalone features
along the way (Google Wallet export, GBP as the default currency, the
Inventory tag filter) and a post-Phase-14 follow-up widening the Full
Backup format. Nothing upstream was rewritten — every phase adds new
models, views, and migrations alongside the existing ones, so this fork
stays a strict superset of upstream and can be rebased/updated against it
at any time.
