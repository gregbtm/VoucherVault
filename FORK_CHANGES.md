...
fifteen rounds of additive features, from Phase 1 (a REST API) through
Phase 14 (offline cache, OpenAI OCR, scheduled backups, an .ics calendar
feed, bulk actions, and n8n integration) and Phase 15 (a documentation and
illustration pass over everything that came before it), plus several
standalone features along the way (Google Wallet export, GBP as the
default currency, the Inventory tag filter) and a post-Phase-14 follow-up
widening the Full Backup format. Nothing upstream was rewritten — every
phase adds new models, views, and migrations alongside the existing ones,
so this fork stays a strict superset of upstream and can be
